const TextInput = ({title, placeholder, multi, date, time, password, state, setState, required, disabled}) => {
    ...
    return <div className='flex flex-col w-full'>
        ...
        {time? <input type="time" onChange={handleChange} value={state} className='bg-white border-1 border-LightColor rounded-lg p-3 ring-0 focus:ring-0 focus:border-LightColor' /> :
          ...
          <input type={password? 'password': 'text'} onChange={handleChange} value={state} placeholder={placeholder} disabled={disabled} className='bg-white border-1 border-LightColor rounded-lg p-3 ring-0 focus:ring-0 focus:border-LightColor' />}
    </div>
  }
